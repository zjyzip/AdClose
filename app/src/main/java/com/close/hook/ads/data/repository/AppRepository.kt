...
class AppRepository(private val packageManager: PackageManager) {
    ...
    companion object {
        ...
        private const val RECENT_UPDATE_THRESHOLD_MS = 30L * 24 * 60 * 60 * 1000
    }
        ...
        val appList = coroutineScope {
            pkgs.map { pkg ->
                async {
                    ...
                    val isEn = try {
                        packageManager.getApplicationEnabledSetting(pkg.packageName) !=
    ...
    fun filterAndSortApps(apps: List<AppInfo>, filter: AppFilterState): List<AppInfo> {
        ...
        return apps.asSequence()
            .filter { app ->
                ...
                if (filter.showUpdated && (now - app.lastUpdateTime > RECENT_UPDATE_THRESHOLD_MS)) return@filter false
