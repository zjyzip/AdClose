...
public class RequestHook {
	...
	private static boolean shouldBlockRequest(String host) {
		try {
			return urlBlockCache.get(host, () -> {
				boolean isBlocked = BLOCKED_LISTS.stream().anyMatch(host::contains);
				sendBlockedRequestBroadcast(isBlocked ? "block" : "pass", host.contains(":") ? " HTTP" : " DNS",
						isBlocked, host);
				return isBlocked;
			});
